...
class entryExitPointer_Dynamic_perEntryMaxLoss:
    ...
    def markEntryPoint(self) :
        ...
        # 判斷停損點是否有值
        isStopLossExist = ~np.isnan(self.pairsPriceData['stopLossCurrent'].iloc[self.index])
        isStopLossAboveZero = (self.pairsPriceData['stopLossCurrent'].iloc[self.index] > 0)
        isStopLossReasonable = isStopLossExist & isStopLossAboveZero
        ...
        self.positionHold = np.where(isSpreadPos &  entryCondAfterStopLoss & isStopLossReasonable & isPercentileBelowThreshold, -1, np.where(isSpreadNeg & entryCondAfterStopLoss & isStopLossReasonable & isPercentileBelowThreshold , 1, 0))
    ...
    def startGetPortfolio(self):
        ...
        if (self.positionHold != 0) & (~self.isExitPoint) & (~self.isCurrentHoldPortfolio): 
            ...
            # 最大停損點
            self.maxLoss = abs(self.pairsPriceData['stopLossCurrent'].iloc[self.index])

            # 使用最大停損點逆推最大停利點
            self.exitCondStopLoss = (np.absolute(spreadValueWhenEntry) + self.maxLoss)
            self.exitCondStopProfit = (np.absolute(spreadValueWhenEntry) - self.maxLoss * (1/self.riskReward))
    ...
    def markExitPoint(self) :
        ...
        if (self.index > 0) & (self.isCurrentHoldPortfolio) :
            ...
            # 出場條件依目前的 ZS 更新
            newexitCondStopLoss = np.absolute(self.spreadValueCurrent) + self.maxLoss
