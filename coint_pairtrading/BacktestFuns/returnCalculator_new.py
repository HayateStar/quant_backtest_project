...
class returnCalculator :
    ...
    #     token2Value = 150
    #     token2Qty = 150/self.extryExitPtData['token2_close'].iloc[index]
    #     token1Qty = token2Qty*self.beta

    #     token1TransQty = self.transformQty(self.token1, token1Qty[0])
    #     token2TransQty = self.transformQty(self.token2, token2Qty)
    ...
    def calculateWeightedReturn(self) :
        ...
        # print(self.token1, self.token2, self.longPositionToken1_Start,self.shortPositionToken2_Start )
    ...
    def openPosition(self, index) : 
        ...
        isExitPoint = self.extryExitPtData['isExitPoint'].iloc[index]

        # 特例：若資產從 i = 0 開始持有
        #if (self.previousPosition != 0) & (index == 1) & (self.startIndex == -1):
        # if (self.currentPosition != 0) & (index == 0) & (self.startIndex == -1)  & (~isExitPoint):

        #     self.startIndex = 0

        # 若手上未有資產且出現入場點，則開倉。
        #if (self.currentPosition != 0) & (self.previousPosition == 0) & (self.startIndex == -1):
        if (self.currentPosition != 0)  & (self.startIndex == -1) & (~isExitPoint):
    ...
    def closePositionMidMonth(self, index) :
        ...
        # if isExitPoint :
        #     print(self.currentPosition, self.startIndex)
        #if (self.currentPosition == 0) & (self.previousPosition != 0) & (self.startIndex != -1):
        if (self.currentPosition != 0) & (isExitPoint) & (self.startIndex != -1):
            ...
            #if index == 1 :
                #self.getLongShortPortfolio(index = index, isStartIndex = False)
            ...
            #else : 
                #self.getLongShortPortfolio(index = index - 1, isStartIndex = False)
    ...
    def closePositionEndofMonth(self, index) :
        ...
        if (index == len(self.extryExitPtData)-1) & (self.startIndex != -1) :
            ...
            # # 計算入場點 Long/Short 資產價值
            # self.getLongShortPortfolio(index = self.startIndex, isStartIndex = True)
            # self.startDate = self.extryExitPtData['datetime'].iloc[self.startIndex]
    ...
    def recordEachTrade(self) : 
        ...
        returnRecordRow = {   
            ...
            'token1_Qty' : [self.token1TransQty],
            'token2_Qty' : [self.token2TransQty],
            'beta' : [self.beta[0]],
    ...
    def main(self) :
        ...
        for i in range(0, len(self.extryExitPtData)):
